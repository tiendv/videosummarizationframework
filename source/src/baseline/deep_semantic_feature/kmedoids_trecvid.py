...
if __name__ == "__main__":
    ...
    for person in ['janine','ryan','stacey']:
        ...
        for video_name in list_video_name_bbc:
            feature_video = np.squeeze(np.load('/mmlabstorage/workingspace/VideoSum/videosummarizationframework/data/BBC_processed_data/frames_feature/inceptionv3/'+video_name+'_inceptionv3_1.npy'))
            print (video_name)
            if os.path.exists('/mmlabstorage/workingspace/VideoSum/videosummarizationframework/data/BBC_processed_data/VSUM_TRECVID/bbc_person_segment/'+video_name+'/'+person+'.txt') :
                with open('/mmlabstorage/workingspace/VideoSum/videosummarizationframework/data/BBC_processed_data/VSUM_TRECVID/bbc_person_segment/'+video_name+'/'+person+'.txt','r') as person_shot:
                    ...
                    for line in Lines:
                        ...
                        feature.append(np.mean(np.array(feature_video[start:end]),axis = 0))
        feature = np.array(feature)
        for k in [5,10,15,20]:
            selected = run_kmedoids(feature,k,person)
            print (selected)
            with open('/mmlabstorage/workingspace/VideoSum/videosummarizationframework/data/BBC_processed_data/VSUM_TRECVID/bbc_segment_time/bbc_inceptionv3_kmedoids/'+str(k)+'/'+person+'.txt','w') as file_save:
                for i in selected:
