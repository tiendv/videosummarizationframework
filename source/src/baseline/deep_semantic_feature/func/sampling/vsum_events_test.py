...
from func.dataset.summe_events_test import SUMME
import numpy as np
...
class VSUM(gm_submodular.DataElement):
    ...
    def __init__(self, videoID, model,fps,datatype,duration,path_npy,total_frames,
                 dataset='summe',
                 featType='vgg',
                 seg_l=4):
        # load dataset data
        self.dataset = SUMME(videoID,fps,datatype,duration,path_npy,total_frames)
...
def encodeSeg(data, model, seg_size=4):
    ...
    for i in range(feat_temp.shape[0]-5):
        temp = []
        for j in range(feat_temp.shape[1]):
            temp.append((feat_temp[i][j]+feat_temp[i+1][j]+feat_temp[i+2][j]+feat_temp[i+3][j]+feat_temp[i+4][j])/5)
        feat.append(temp)
    feat=np.array(feat)
    enc_x = model(feat)
