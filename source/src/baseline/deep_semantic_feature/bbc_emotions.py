...
#    v_id = (v_id.split("/")[-1]).split(".")[0]
    ...
    dict_refer= {}
    video_done=[]
    with open("logemotion.txt","r") as log:
        Lines = log.readlines() 
        for line in Lines:
    ...
    #    duration=video.get(cv2.CAP_PROP_POS_MSEC)
        ...
        with configuration.using_config('train', False):
            with chainer.no_backprop_mode():
                vsum = VSUM( v_id, model,fps,datatype,duration,path_npy,int(total_frames),path_reference)
        ...
        with open(os.path.join(path_save_txt,real_name.replace(".mp4",""),real_name.replace(".mp4","")+".txt"),"w+") as f:
            for i in range(len(label)) :
                ...
                if i+1 == len(label) and in_seg ==True:
                    ...
                    time_end = 0
                    in_seg = False
            with open("logemotion.txt","a") as log:
...
if __name__ == "__main__":
    ...
    ### config this code
    path_save_txt = "/home/workingspace/VideoSum/videosummarizationframework/data/BBC_processed_data/time_shots_bbc/emotion_seg_vsum_dsf_fix"
    path_video = "/home/datasets/TRECVID/TRECVID_BBC_EastEnders/videos"
    datatype= 'bbc'  ###tvsum or summe or bbc
    path_npy = "/home/workingspace/VideoSum/videosummarizationframework/data/BBC_processed_data/events_emotion"
    path_reference = "/home/workingspace/VideoSum/videosummarizationframework/data/BBC_processed_data/reference_bbc/master_shot_reference.txt"
    ###
