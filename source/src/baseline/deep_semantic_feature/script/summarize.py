    ...
    for v_id in video_id:
        with configuration.using_config('train', False):
            with chainer.no_backprop_mode():
                vsum = VSUM(v_id,15, model, dataset=d_name, seg_l=seg_l)  #(Cooking,model,summe,seg_l=5)
                fnum = int(vsum.dataset.data['fps'])
                vsum = VSUM(v_id,fps, model, dataset=d_name, seg_l=seg_l)  #(Cooking,model,summe,seg_l=5)
