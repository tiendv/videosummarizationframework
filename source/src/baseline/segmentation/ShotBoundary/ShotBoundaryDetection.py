...
class ShotBoundaryDetection:
    ...
    def __set_video(self, vid):
        ...
        self.__fps = self._vid.get(cv2.CAP_PROP_FPS)

        # Shot boundary attributes
        self.sb = []

    def get_total_frame_and_fps(self):
    ...
    def __detect_multi_step_comparison_scheme(self,
        ...
        def calc_phi_eta(alg_atr_l):
            ...
            mu = []
            for n in range(len(histograms)):
                a=np.array([calc_local_mean()] * (alg_atr_l + 1))
                a = np.squeeze(a)
                #print(a)
                mu.append(a)
            # noinspection PyShadowingNames
            #print("Thinh: ",(mu),type(sigma),sigma.shape)
            mu = np.asarray(mu)
            mu = np.matrix(mu)
    ...
    def detect(self, set_progress=None, finish=None):
        ...
        while cut_index < len(cuts) and gradual_index < len(gradual_transitions):
            if cuts[cut_index] < gradual_transitions[gradual_index][0]:
                ...
                list_begin.append(cuts[cut_index])
                list_end.append(-1)
                cut_index += 1
            else:
                self.sb.append(#{'transition': 'gradual',
                               # 'start_frame': 
                    {gradual_transitions[gradual_index][0],
                                #'start_time': ms_to_time(self.frame_num_to_timestamp(gradual_transitions[gradual_index][0])),
                                #'end_frame': 
                    gradual_transitions[gradual_index][1]}
                                #'end_time': ms_to_time(                                    self.frame_num_to_timestamp(gradual_transitions[gradual_index][1]))}
                )
                list_begin.append(gradual_transitions[gradual_index][0])
                list_end.append(gradual_transitions[gradual_index][1])
...
def write_to_file(data_dir,out_dir,fps,list_begin,list_end):
    pass
if __name__ == "__main__":
    ...
    path='test.mp4'
    sbd.open_video(path)
    print(sbd.get_total_frame_and_fps())
