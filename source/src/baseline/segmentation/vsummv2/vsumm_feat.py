...
def save_keyframes(frame_indices, summary_frames):
    ...
    if int(sys.argv[6])==1:
        ...
        namefile = os.path.splitext(name)[0]
        make_folder(sys.argv[7],namefile)
        #out_file=open(sys.argv[7]+"frame_indices_"+sys.argv[8]+'_'+str(num_centroids)+"_"+str(sampling_rate)+".txt",'w')
        out_file=open( os.path.join(sys.argv[7],namefile,namefile)+'.txt','w')
        for idx in frame_indices:
            out_file.write(str(idx*sampling_rate)+'.jpg\n')
        print ("Saved indices")
