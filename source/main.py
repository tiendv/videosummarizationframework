...
def calc_score(list_begin,list_ending):
    ...
    for _ in range(len(list_begin)):
        list_score.append(round(random.randint(0,50)/10,1))
    return list_score


def main():
    path_video = "src/visualization/static/TVSum50/ydata-tvsum50-v1_1/video/cjibtmSLxQ4.mp4"
    #shot detection
    ...
    # create_json4shots(name_video,list_begin,list_ending,list_score,"./")

    #adapt knapsack to select the shots for summarize
    result = selection_shot_knapsack(list_begin,list_ending,list_score)

    # create_json_selection(name_video,list_begin,list_ending,None,"./")
