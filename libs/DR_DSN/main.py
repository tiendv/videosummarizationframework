...
#            seq = (np.zeros((300,1024))).float().cuda()

#            np.save("/mmlabstorage/workingspace/VideoSum/trivlm/rethinking-evs/data/feature/googlenet_summe/"+dataset[key]['video_name'][...],seq)
            ...
            machine_summary,frames_score = vsum_tools.generate_summary(probs, cps, num_frames, nfps, positions)
            """
            ...
            total_frames = 0
            duration = 0
            data = pandas.read_csv(os.path.join("/mmlabstorage/workingspace/VideoSum/videosummarizationframework/data/input_data/SumMe.csv"),header=None)
            for i in range(data.shape[0]):
                if data[0][i] == str(dataset[key]['video_name'][...])+".mp4":
                    fps = float(data[2][i])
                    total_frames = int(float(data[3][i]))
                    duration = float(data[4][i])
#            write_data(machine_summary,fps,"/mmlabstorage/workingspace/VideoSum/videosummarizationframework/data/TVSum_processed_data/time_segment/vsumm-reinforce",list_name[key_idx])
            """
            """
            path_save = '/mmlabstorage/workingspace/VideoSum/videosummarizationframework/data/TVSum_processed_data/time_shots_tvsum50/vsumm-reinforce'
            start = 0
            end = 15
            if not os.path.isdir(os.path.join(path_save,list_name[key_idx])):
                os.makedirs(os.path.join(path_save,list_name[key_idx]))
            with open(os.path.join(path_save,list_name[key_idx],list_name[key_idx]+".txt"),"w") as f:
            ...
            """
            eval_metric = 'avg'
            fm, pre, recall = vsum_tools.evaluate_summary(machine_summary, user_summary, eval_metric)
            """
            f1.append(fm)
            p.append(pre)
            r.append(recall)
            if str(dataset[key]['video_name'][...]) not in list_videos:
                list_videos.append(str(dataset[key]['video_name'][...]))
            """
            fms.append(fm)
        ...
        import json
        print (json_summe)
        with open('/mmlabstorage/workingspace/VideoSum/videosummarizationframework/source/src/visualization/static/evaluation/SumMe/KTS_DR-DSN_Knapsack/KTS_DR-DSN_Knapsack.json', 'w') as f:
