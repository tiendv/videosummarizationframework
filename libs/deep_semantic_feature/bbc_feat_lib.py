...
def run_vsum_dsf(path_save_txt,path_video,datatype,path_npy,path_reference,seg_l,feat_type,list_name_video):
    ...
    dict_refer= {}
    video_done=[]
    with open("/mmlabstorage/workingspace/VideoSum/videosummarizationframework/source/src/baseline/deep_semantic_feature/log.txt","r") as log:
        Lines = log.readlines() 
        for line in Lines:
        ...
        with open(os.path.join(path_save_txt,real_name.replace(".mp4",""),real_name.replace(".mp4","")+".txt"),"w+") as f:
            for i in range(len(label)) :
                ...
                if i+1 == len(label) and in_seg ==True:
                    ...
                    time_end = 0
                    in_seg = False
            with open("/mmlabstorage/workingspace/VideoSum/videosummarizationframework/source/src/baseline/deep_semantic_feature/log.txt","a") as log:
