...
class AONet:
    ...
    def eval(self, keys, results_filename=None):
        ...
        with torch.no_grad():
            for i, key in enumerate(keys):
                ...
                """
                try:
                    print(str(np.char.decode(data['video_name'][()])))
    ...
    def eval_summary(self, machine_summary_activations, test_keys, results_filename=None, metric='tvsum', att_vecs=None):
        ...
        for key_idx, key in enumerate(test_keys):
            ...
            machine_summary , frames_score = generate_summary(probs, cps, num_frames, nfps, positions)

            eval_metric = 'avg'
            fm, pre, recall = evaluate_summary(machine_summary, user_summary, eval_metric)


            #str(np.char.decode(d['video_name'][()]))
            """
            path_gt_tvsum = "/mmlabstorage/datasets/TVSum50/ydata-tvsum50-v1_1/ydata-tvsum50-matlab/matlab/ydata-tvsum50.mat"
            f = h5py.File(path_gt_tvsum,'r')
            data = f.get('video')
            ...
            print (list_name[key_idx])
#            np.save("/mmlabstorage/workingspace/VideoSum/trivlm/rethinking-evs/label/tvsum_vasnet_knapsack/"+list_name[key_idx],machine_summary)

            f1.append(fm)
            p.append(pre)
            r.append(recall)

            if list_name[key_idx] not in list_videos:
                list_videos.append(list_name[key_idx])
            """
            """
            fps = 0
            total_frames = 0
            duration = 0
            data = pandas.read_csv(os.path.join("/mmlabstorage/workingspace/VideoSum/videosummarizationframework/data/input_data/TVSum.csv"),header=None)
            for i in range(data.shape[0]):
                if data[0][i] == list_name[key_idx] +".mp4":
                    fps = float(data[2][i])
                    total_frames = int(float(data[3][i]))
                    duration = float(data[4][i])
            """
#            np.save("/mmlabstorage/workingspace/VideoSum/trivlm/rethinking-evs/data/VASNet/"+list_name[key_idx],frames_score)
#            write_data(machine_summary,fps,"/mmlabstorage/workingspace/VideoSum/videosummarizationframework/data/TVSum_processed_data/time_segment/KTS_VASNet_Knapsack",list_name[key_idx])
            """
            path_save = '/mmlabstorage/workingspace/VideoSum/videosummarizationframework/data/TVSum_processed_data/time_shots_tvsum50/VASNet_score'
            start = 0
            end = 15
            if not os.path.isdir(os.path.join(path_save,str(np.char.decode(d['video_name'][()])))):
                os.makedirs(os.path.join(path_save,str(np.char.decode(d['video_name'][()]))))
            with open(os.path.join(path_save,str(np.char.decode(d['video_name'][()])),str(np.char.decode(d['video_name'][()]))+".txt"),"w") as f:
                for i in range(int(len(frames_score)/15)+1):
                    print (end)
                    if start == len(frames_score):
                        break
                    print (sec2time(start/fps)+ " " + sec2time(end/fps) + " " +str(frames_score[start]))
                    f.write(sec2time(start/fps)+ " " + sec2time(end/fps) + " " +str(frames_score[start])+"\n")
                    start = end
                    if end + 15 >= len(frames_score):
                        end = len(frames_score)
                    else:
                        end += 15
            print (frames_score)
            print (total_frames)
        ...
        import json
        print (json_summe)
        with open('/mmlabstorage/workingspace/VideoSum/videosummarizationframework/source/src/visualization/static/evaluation/TVSum/KTS_VASNet_Knapsack/KTS_VASNet_Knapsack.json', 'w') as f:
