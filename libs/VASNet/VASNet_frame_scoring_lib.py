...
def get_VASNet_score(features,path_pretrained_model=''):

    ao = AONet(hps)
    ao.initialize()
    print("Loading model:", path_pretrained_model.split("/")[-1])
    ao.load_model(path_pretrained_model)
    scores = ao.get_score(features)
    return scores
